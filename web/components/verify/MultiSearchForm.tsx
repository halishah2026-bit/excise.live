...
interface SearchType { value: string; label: string; placeholder: string; mono?: boolean; }
...
export default function MultiSearchForm({ title, subtitle, endpoint, searchTypes, color }: Props) {
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 sm:p-6">
          ...
          <form onSubmit={handleSearch} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {searchTypes.map(t => (
                  <button key={t.value} type="button"
                    ...
                    className={`py-2.5 px-4 rounded-xl text-sm font-semibold transition-all ${searchType === t.value ? `${color} text-white shadow-sm` : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                  >
                    {t.label}
                  </button>
                ))}
            ...
            <button type="submit" disabled={loading}
              className={`w-full ${color} disabled:opacity-60 text-white font-bold py-3.5 rounded-xl transition-all flex items-center justify-center gap-2 text-base shadow-sm hover:shadow-md hover:opacity-90`}>
        ...
        {searched && (
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 sm:p-6">
            ...
            <VehicleResult found={result?.found} data={result?.data} service={title.split(' ')[0]} />
          </div>
        )}
      </div>
    </DashboardLayout>
