...
export default function NonCustomVerification() {
  ...
  const handleSearch = async (e: FormEvent) => {
    ...
    try {
      ...
      const { data } = await API.post('/verify/noncustom', payload);
      setResult(data);
  ...
  return (
    <DashboardLayout title="Non-Custom Vehicle Database" subtitle="Check non-custom paid vehicles by Chassis or Engine No.">
      <div className="max-w-2xl mx-auto space-y-6">
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 sm:p-6">
          ...
          <div className="mb-4 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 text-sm text-amber-900">
            Vehicles in this database have not paid customs duty and may be impounded.
          </div>
          <form onSubmit={handleSearch} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {searchTypes.map(t => (
                  <button key={t.value} type="button"
                    onClick={() => { setSearchType(t.value); setSearchValue(''); }}
                    className={`py-3 px-4 rounded-xl text-sm font-semibold transition-all ${searchType === t.value ? 'bg-purple-600 text-white shadow-sm' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                    {t.label}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <input type="text" required value={searchValue}
                ...
                className="w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-purple-500 bg-gray-50 font-mono text-base sm:text-lg tracking-wider uppercase"
              />
            </div>
            <button type="submit" disabled={loading}
              className="w-full bg-purple-600 hover:bg-purple-700 disabled:opacity-60 text-white font-bold py-3.5 rounded-xl transition-all flex items-center justify-center gap-2 text-base shadow-sm hover:shadow-md">
              {loading ? <><div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin"/>Searching...</> : <><svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"/></svg>Check Vehicle</>}
            </button>
          </form>
        </div>
        ...
        {searched && (
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 sm:p-6">
            <h3 className="font-bold text-gray-900 mb-5 text-lg border-b border-gray-100 pb-3">Check Results</h3>
            <VehicleResult found={result?.found} data={result?.data} service="Non-Custom" />
          </div>
        )}
      </div>
    </DashboardLayout>
