...
const searchTypes = [
  { value: 'registration', label: 'Registration No.', placeholder: 'e.g. AJK-1234', mono: true },
  { value: 'cnic', label: 'CNIC', placeholder: 'e.g. 13301-1234567-1', mono: false },
];

export default function AJKVerification() {
  ...
  const handleSearch = async (e: FormEvent) => {
    ...
    try {
      ...
      const { data } = await API.post('/verify/ajk', payload);
      setResult(data);
  ...
  return (
    <DashboardLayout title="AJK Vehicle Verification" subtitle="Search by CNIC or Registration Number">
      <div className="max-w-2xl mx-auto space-y-6">
          ...
          <form onSubmit={handleSearch} className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                {searchTypes.map(t => (
                  <button key={t.value} type="button"
                    ...
                    className={`py-3 px-4 rounded-xl text-sm font-semibold transition-all ${searchType === t.value ? 'bg-sky-600 text-white shadow-sm' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}>
                    {t.label}
                  </button>
                ))}
              </div>
            ...
            <div>
              ...
              <input type="text" required value={searchValue}
                onChange={e => setSearchValue(activeType.mono ? e.target.value.toUpperCase() : e.target.value)}
                placeholder={activeType.placeholder}
                className={`w-full px-4 py-3 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-sky-500 bg-gray-50 ${activeType.mono ? 'font-mono text-base sm:text-lg tracking-wider uppercase' : ''}`}
              />
            </div>
            <button type="submit" disabled={loading}
              className="w-full bg-sky-600 hover:bg-sky-700 disabled:opacity-60 text-white font-bold py-3.5 rounded-xl transition-all flex items-center justify-center gap-2 text-base shadow-sm hover:shadow-md">
        ...
        {searched && (
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 sm:p-6">
            ...
            <VehicleResult found={result?.found} data={result?.data} service="AJK" />
          </div>
        )}
      </div>
    </DashboardLayout>
